...
const Sidebar = ({ isOpen, onToggle }) => {
  ...
  return (
    <aside
      className={`glass-sidebar fixed left-0 top-0 h-screen z-40 flex flex-col transition-all duration-300
      ${collapsed ? "w-20" : "w-64"}
      lg:translate-x-0 
      ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
    >
      {/* Logo */}
      <div className="flex items-center justify-between gap-3 px-5 py-6 border-b border-surface-200/50 dark:border-surface-700/50">
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 flex items-center justify-center shadow-md hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors hidden lg:flex"
      >
        {collapsed ? (
