...
const OrderHistory = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      {loading ? (
        <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-32 rounded-2xl bg-surface-200 dark:bg-surface-800 animate-pulse" />)}</div>
      ) : orders.length === 0 ? (
        <div className="glass-card p-12 text-center">
          <Package className="w-16 h-16 mx-auto text-surface-300 mb-4" />
          <h3 className="text-lg font-semibold mb-2">No orders yet</h3>
          <p className="text-surface-500">Your order history will appear here.</p>
        </div>
      ) : (
            ...
            <div key={order._id} className="glass-card p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <span className={statusColors[order.status] || 'badge-neutral'}>{order.status}</span>
                  <span className="text-xl font-bold text-primary-600 dark:text-primary-400">₹{order.total?.toFixed(2)}</span>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {order.items?.map((item, i) => (
                  <div key={i} className="flex items-center gap-3 bg-surface-50 dark:bg-surface-800 rounded-xl p-3">
