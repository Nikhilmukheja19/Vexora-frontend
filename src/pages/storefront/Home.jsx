    ...
    <div className="min-h-screen">
      ...
      <section className="store-hero relative py-12 sm:py-16 lg:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute w-48 sm:w-64 h-48 sm:h-64 bg-white/10 rounded-full bottom-10 left-10 animate-float" style={{animationDelay:'2s'}} />
        </div>
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <p className="text-base xs:text-lg sm:text-xl lg:text-2xl store-hero-subtitle mb-6 sm:mb-8 max-w-2xl mx-auto px-2">
            {business?.description || 'Discover our amazing collection of products.'}
          </p>
          <Link 
            to={`/store/${slug}/shop`} 
        ...
        <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-3 xs:gap-4 mb-8 sm:mb-10">
              <div>
                <h2 className="text-2xl xs:text-3xl sm:text-4xl lg:text-5xl font-bold">Featured Products</h2>
                <p className="text-xs xs:text-sm sm:text-base text-surface-500 mt-1 sm:mt-2">Handpicked just for you</p>
              </div>
              <Link 
                to={`/store/${slug}/shop`} 
                  ...
                  <div className="relative aspect-square bg-surface-100 dark:bg-surface-800 overflow-hidden">
                    <Link to={`/store/${slug}/product/${product._id}`} className="block w-full h-full">
                      {product.images?.[0] ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-surface-300"><ShoppingBag className="w-12 sm:w-16 h-12 sm:h-16" /></div>
                      )}
                    </Link>
                  </div>
                  <div className="p-3 sm:p-4 lg:p-5">
                    <Link to={`/store/${slug}/product/${product._id}`} className="block">
                      <p className="text-2xs xs:text-xs sm:text-sm text-surface-500 uppercase tracking-wide">{product.category}</p>
                      <h3 className="font-bold text-sm xs:text-base sm:text-lg mt-1 sm:mt-1.5 line-clamp-2 hover:store-text-primary transition-colors">{product.name}</h3>
                      <p className="text-surface-500 text-xs xs:text-sm mt-0.5 sm:mt-1 line-clamp-2">{product.description}</p>
                    </Link>
                    <div className="flex items-center justify-between mt-3 sm:mt-4 gap-2">
                      <span className="text-lg xs:text-xl sm:text-2xl font-bold store-text-primary">₹{product.price}</span>
                      <button 
      ...
      <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 bg-surface-100/50 dark:bg-surface-900/50">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-2xl xs:text-3xl sm:text-4xl lg:text-5xl font-bold mb-8 sm:mb-12">Why Shop With Us?</h2>
          <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {[
              ...
              <div key={i} className="glass-card p-4 sm:p-6 lg:p-8 text-center">
                <span className="text-4xl sm:text-5xl lg:text-6xl block mb-3 sm:mb-4">{item.icon}</span>
                <h3 className="text-base sm:text-lg lg:text-xl font-bold mb-2">{item.title}</h3>
                <p className="text-xs xs:text-sm sm:text-base text-surface-500">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
