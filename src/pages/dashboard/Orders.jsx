            ...
            <tbody>
              {loading ? (
                [1, 2, 3].map((i) => (
                  <tr key={i} className="table-row">
                    <td colSpan={8} className="px-6 py-4">
                      <div className="h-6 bg-surface-200 dark:bg-surface-700 rounded animate-pulse" />
                    </td>
                  </tr>
                ))
              ) : orders.length === 0 ? (
                <tr>
                  <td
                    colSpan={8}
                    className="px-6 py-12 text-center text-surface-400"
                  >
                ...
                orders.map((order) => (
                  <tr key={order._id} className="table-row">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={
                          statusColors[order.status] || "badge-neutral"
                        }
                      >
                        {order.status}
                      </span>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={
                          paymentColors[order.paymentStatus] || "badge-neutral"
                        }
                      >
                        {order.paymentStatus}
                      </span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                          ...
                          {openDropdown === order._id && (
                            <div className="absolute right-0 mt-1 w-40 glass-card p-1 z-10">
                              {[
                                "confirmed",
                                "processing",
                                "shipped",
            ...
            <div className="p-6 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {selectedOrder.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-4 p-3 bg-surface-50/50 dark:bg-surface-800/30 rounded-xl"
                      >
                        <img
                          src={item.image}
                          alt={item.name}
              ...
              <div className="bg-surface-900 text-white p-6 rounded-2xl space-y-3">
                ...
                <div className="flex justify-between text-sm opacity-60">
                  ...
                  <span>
                    ₹
                    {(selectedOrder.shippingFee + selectedOrder.tax)?.toFixed(
                      2,
                    )}
                  </span>
                </div>
